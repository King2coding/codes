# cml_rainfall_retrieval_stats_wetdry_core_bas_202605.py
#
...
def build_15min_timeseries(df_clean: pd.DataFrame) -> pd.DataFrame:
    ...
    # Propagate static metadata into regularized filler rows.
    # When regularize_grid=True, rows with src_present=False are inserted to keep
    # a strict 15-min timeline. These rows need the link-level metadata for
    # downstream frequency-specific wet/dry classification, rain conversion, and
    # gridding. The signal itself remains NaN on filler rows.
    static_cols = [
        "Frequency", "PathLength", "XStart", "YStart", "XEnd", "YEnd",
        "pol", "semantics"
    ]
    for col in static_cols:
        if col in ts.columns:
            ts[col] = ts.groupby("link_id", sort=False)[col].transform(lambda s: s.ffill().bfill())
...
# =============================================================================
# 2B. Statistics-based wet/dry-first retrieval method
# =============================================================================


def assign_frequency_family(freq_ghz) -> str:
    """
    Assign each link to a moderately narrow frequency family.

    Why this is used:
    - Rain-induced CML signal variability depends on microwave frequency.
    - Exact-frequency grouping can create tiny unstable classes.
    - Very broad frequency bins can hide frequency-dependent behavior.

    These families are tuned to the AT Ghana frequency clusters currently seen
    in the raw files. If another network is used, check the frequency inventory
    and update the intervals if needed.
    """
    if pd.isna(freq_ghz):
        return "unknown"
    ...
    if 5.8 <= f < 6.6:
        return "6GHz_family"
    if 7.5 <= f < 8.05:
        return "7p8GHz_family"
    if 8.05 <= f < 8.5:
        return "8p1GHz_family"
    if 17.5 <= f < 18.3:
        return "18GHz_family"
    if 18.5 <= f < 19.3:
        return "19GHz_family"
...
def add_frequency_family(ts_15: pd.DataFrame) -> pd.DataFrame:
    """Add frequency-family labels after filling static link metadata."""
    out = ts_15.copy()
    static_cols = [
        "Frequency", "PathLength", "XStart", "YStart", "XEnd", "YEnd",
        "pol", "semantics"
    ...
    for col in static_cols:
        if col in out.columns:
            out[col] = out.groupby("link_id", sort=False)[col].transform(lambda s: s.ffill().bfill())
    out["freq_family"] = out["Frequency"].apply(assign_frequency_family)
    return out


def _rolling_std_by_link(
    ts_15: pd.DataFrame,
    window_bins: int = 10,
    min_periods: Optional[int] = None,
    signal_col: str = "sig_db",
) -> pd.Series:
    """
    Trailing rolling standard deviation of the standardized signal per link.

    For 15-min data, window_bins=10 is 150 minutes, matching the scale used in
    Kingsley's earlier signal-statistics wet/dry work. The decision is assigned
    to the current timestamp only, which keeps the method near-real-time safe.
    """
    if min_periods is None:
        min_periods = max(3, int(np.ceil(window_bins / 2)))

    def _one(g):
        s = pd.to_numeric(g[signal_col], errors="coerce").astype(float)
        return s.rolling(window=window_bins, min_periods=min_periods).std()

    return (
        ts_15.sort_values(["link_id", "time"])
        .groupby("link_id", sort=False, group_keys=False)
        .apply(_one)
        .reindex(ts_15.index)
    )


def estimate_std_thresholds_by_frequency_family(
    df: pd.DataFrame,
    std_col: str = "sig_std_db",
    family_col: str = "freq_family",
    percentile: float = 95.0,
    min_samples_per_family: int = 500,
) -> Dict[str, float]:
    """
    Estimate wet/dry thresholds from rolling signal standard deviations.

    Thresholds are computed separately within each frequency family. If a family
    has too few valid samples, the global threshold is used as a fallback.
    """
    vals_all = pd.to_numeric(df[std_col], errors="coerce").dropna()
    if vals_all.empty:
        raise ValueError("No valid rolling standard deviations available for threshold estimation.")

    global_thr = float(np.nanpercentile(vals_all.values, percentile))
    thresholds = {"global": global_thr}

    for fam, g in df.groupby(family_col, dropna=False):
        fam_name = str(fam)
        vals = pd.to_numeric(g[std_col], errors="coerce").dropna().values
        if len(vals) >= min_samples_per_family:
            thresholds[fam_name] = float(np.nanpercentile(vals, percentile))
        else:
            thresholds[fam_name] = global_thr

    return thresholds


def _run_length_true(mask: np.ndarray) -> np.ndarray:
    """Return run length for each True element; False elements get 0."""
    m = np.asarray(mask, dtype=bool)
    out = np.zeros(m.size, dtype=int)
    if m.size == 0:
        return out
    start = None
    for i, val in enumerate(m):
        if val and start is None:
            start = i
        if (not val or i == m.size - 1) and start is not None:
...
def add_temporal_persistence_flags(
    df: pd.DataFrame,
    wet_col: str = "wet_stat_initial",
    min_wet_run_bins: int = 2,
) -> pd.DataFrame:
    """
    Add temporal support for wet decisions.

    A single isolated 15-min wet flag can be caused by noise or a signal jump.
    We therefore flag wet periods that persist for at least min_wet_run_bins.
    The default does not delete isolated wet flags by itself; it exposes the
    support information for wet_final/confidence/gridding decisions.
    """
    parts = []
    for _, g in df.sort_values(["link_id", "time"]).groupby("link_id", sort=False):
        g = g.copy()
        wet = g[wet_col].fillna(False).astype(bool).values
        run_len = _run_length_true(wet)
        g["wet_run_length_bins"] = run_len
        g["wet_temporal_support"] = wet & (run_len >= int(min_wet_run_bins))
        parts.append(g)
    return pd.concat(parts, ignore_index=True)


def _baseline_from_dry_past_only(
    ...
    min_dry_bins: int = 8,
    ffill_limit_bins: int = 32,
    bfill_limit_bins: int = 4,
) -> Tuple[pd.Series, pd.Series]:
    """
    Estimate baseline from dry-classified samples only.

    This follows the wet/dry-first retrieval logic: dry periods are identified
    first, and the baseline is then built only from those dry samples. A median
    dry baseline (q=0.50) is the default because it is less sensitive to high
    signal jumps than a near-maximum/high-quantile baseline.
    """
    sig_dry = sig_series.where(dry_mask.astype(bool))
    base = sig_dry.rolling(window=win, min_periods=min_dry_bins, closed="left").quantile(q)
    dry_count = sig_dry.notna().rolling(window=pd.Timedelta(win), closed="left").sum()
    base = base.ffill(limit=ffill_limit_bins).bfill(limit=bfill_limit_bins)
    return base, dry_count


def stats_wetdry_Aobs(
    ts_15: pd.DataFrame,
    window_bins: int = 10,              # 10 x 15 min = 150 min
    std_percentile: float = 95.0,       # stricter wet/dry threshold
    min_std_samples_per_family: int = 500,
    min_periods_std: Optional[int] = None,
    min_wet_run_bins: int = 2,
    require_temporal_support_for_wet: bool = False,
    baseline_win: str = "48H",          # RainLINK-inspired long dry-reference window
    baseline_q: float = 0.50,           # dry-only median baseline
    min_dry_bins: int = 8,
    ffill_limit_bins: int = 32,
    require_src_present: bool = True,
) -> Tuple[pd.DataFrame, Dict[str, float]]:
    """
    Statistics-based wet/dry-first attenuation retrieval.

    Candidate method after diagnostics:
      1. Use sig_db, the semantics-safe signal where larger means drier/stronger.
      2. Compute trailing rolling standard deviation per link using a 150-min window.
      3. Derive wet/dry thresholds by narrow frequency family.
      4. Use a stricter high-percentile threshold, currently p95, to reduce false wet flags.
      5. Estimate baseline from dry-classified samples only.
      6. Use a 48H dry-only median baseline, following the RainLINK/pycomlink-style
         idea that dry reference levels should be stable and based on dry periods.
      7. Compute attenuation as baseline_sig_db - sig_db and force dry periods to zero.

    This reduces the risk of baseline inflation seen in the old baseline-first/
    high-quantile method, where short high-signal jumps could become an unrealistic
    dry reference and create persistent false rainfall.
    """
    r = add_frequency_family(ts_15)
    r = r.sort_values(["link_id", "time"]).reset_index(drop=True)

    # Compute link-level rolling signal variability.
    r["sig_std_db"] = _rolling_std_by_link(
        r,
    ...
    # Estimate thresholds per frequency family using source rows only when requested.
    thr_pool = r.copy()
    if require_src_present and "src_present" in thr_pool.columns:
        thr_pool = thr_pool[thr_pool["src_present"].fillna(False).astype(bool)]

    thresholds = estimate_std_thresholds_by_frequency_family(
        thr_pool,
        std_col="sig_std_db",
        family_col="freq_family",
        percentile=std_percentile,
        min_samples_per_family=min_std_samples_per_family,
    )

    r["wet_dry_thr_std_db"] = r["freq_family"].map(thresholds).fillna(thresholds["global"])
    r["wet_stat_initial"] = (
        pd.to_numeric(r["sig_std_db"], errors="coerce") >
        pd.to_numeric(r["wet_dry_thr_std_db"], errors="coerce")
    )
    r["wet_stat_initial"] = r["wet_stat_initial"].fillna(False)

    if require_src_present and "src_present" in r.columns:
        r.loc[~r["src_present"].fillna(False).astype(bool), "wet_stat_initial"] = False

    r = add_temporal_persistence_flags(
        r,
        wet_col="wet_stat_initial",
        min_wet_run_bins=min_wet_run_bins,
    )

    if require_temporal_support_for_wet:
        r["wet_final"] = r["wet_stat_initial"] & r["wet_temporal_support"]
    else:
        # Keep the statistics-based decision but expose temporal support for
        # confidence/gridding. This avoids removing short true convective events
        # too early in sparse CML regions.
        r["wet_final"] = r["wet_stat_initial"]

    # Placeholder spatial support columns. They can be filled later by
    # add_spatial_wetdry_support(), but are present now for transparent outputs.
    r["neighbor_count"] = 0
    r["neighbor_wet_fraction"] = np.nan
    r["wet_spatial_support"] = False

    parts = []
    for lid, g in r.groupby("link_id", sort=False):
        g = g.sort_values("time").copy()
        idx = pd.DatetimeIndex(pd.to_datetime(g["time"], utc=True))
        sig = pd.Series(pd.to_numeric(g["sig_db"], errors="coerce").astype(float).values, index=idx)

        valid_source = np.isfinite(sig.values)
        if require_src_present and "src_present" in g.columns:
            valid_source = valid_source & g["src_present"].fillna(False).astype(bool).values

        dry_mask = (~g["wet_final"].fillna(False).astype(bool).values) & valid_source
        dry_mask = pd.Series(dry_mask, index=idx)

        base, dry_count = _baseline_from_dry_past_only(
            sig,
            dry_mask=dry_mask,
            win=baseline_win,
            q=baseline_q,
            min_dry_bins=min_dry_bins,
            ffill_limit_bins=ffill_limit_bins,
        )

        Aobs_raw = np.maximum(0.0, base.values - sig.values)
        wet = g["wet_final"].fillna(False).astype(bool).values
        Aobs = np.where(wet & np.isfinite(Aobs_raw), Aobs_raw, 0.0)

        g["baseline_sig_db"] = base.values
        g["dry_count_win"] = dry_count.values
        g["A_obs_dB"] = Aobs
        # Keep legacy-style wet column name so rainlink_strict_R can be reused.
        g["wet_rl"] = g["wet_final"].astype(bool)
        parts.append(g)

    out = pd.concat(parts, ignore_index=True)

    # Retrieval confidence: this is a practical support score, not formal uncertainty.
    conf = np.zeros(len(out), dtype=float)
    wet = out["wet_final"].fillna(False).astype(bool).values
    temporal = out["wet_temporal_support"].fillna(False).astype(bool).values
    conf[wet] = 0.50
    conf[wet & temporal] += 0.25
    # spatial term can be updated after add_spatial_wetdry_support()
    out["retrieval_confidence"] = np.clip(conf, 0.0, 1.0)

    return out, thresholds


def add_spatial_wetdry_support(
    df: pd.DataFrame,
    radius_km: float = 25.0,
    min_neighbors: int = 3,
    min_neighbor_wet_fraction: float = 0.50,
    wet_col: str = "wet_final",
) -> pd.DataFrame:
    """
    Add nearby-link support diagnostics for wet/dry decisions.

    In sparse AT Ghana regions, nearby-link evidence may be unavailable. In that
    case we do not automatically force the link dry; we keep the link-level
    decision but expose low spatial support via neighbor_count/wet fraction.
    """
    out = df.copy()
    mid_lon = (pd.to_numeric(out["XStart"], errors="coerce") + pd.to_numeric(out["XEnd"], errors="coerce")) / 2.0
    mid_lat = (pd.to_numeric(out["YStart"], errors="coerce") + pd.to_numeric(out["YEnd"], errors="coerce")) / 2.0
    out["mid_lon"] = mid_lon
    out["mid_lat"] = mid_lat

    meta = (
        out[["link_id", "mid_lon", "mid_lat"]]
        .dropna()
    ...
    if meta.empty:
        ...
        out["neighbor_wet_fraction"] = np.nan
        out["wet_spatial_support"] = False
        return out

    lat0 = np.deg2rad(meta["mid_lat"].median())
    x = meta["mid_lon"].to_numpy(float) * np.cos(lat0) * _KM_PER_DEG
    y = meta["mid_lat"].to_numpy(float) * _KM_PER_DEG
    xy = np.column_stack([x, y])
    tree = cKDTree(xy)
    neigh = tree.query_ball_point(xy, r=radius_km)
    link_ids = meta["link_id"].tolist()
    link_to_neighbors = {
        lid: [link_ids[j] for j in js if link_ids[j] != lid]
        for lid, js in zip(link_ids, neigh)
    }

    parts = []
    for t, g in out.groupby("time", sort=False):
        g = g.copy()
        wet_map = dict(zip(g["link_id"], g[wet_col].fillna(False).astype(bool)))
        counts = []
        fracs = []
        support = []
        for lid in g["link_id"]:
            nbs = [n for n in link_to_neighbors.get(lid, []) if n in wet_map]
            n = len(nbs)
            if n == 0:
                frac = np.nan
                ok = False
            else:
                frac = float(np.mean([wet_map[n] for n in nbs]))
                ok = (n >= min_neighbors) and (frac >= min_neighbor_wet_fraction)
            counts.append(n)
            fracs.append(frac)
            support.append(ok)
        g["neighbor_count"] = counts
        g["neighbor_wet_fraction"] = fracs
        g["wet_spatial_support"] = support
        parts.append(g)

    out = pd.concat(parts, ignore_index=True)

    # Update confidence after spatial support. Keep dry confidence at zero.
    wet = out[wet_col].fillna(False).astype(bool).values
    temporal = out.get("wet_temporal_support", pd.Series(False, index=out.index)).fillna(False).astype(bool).values
    spatial = out["wet_spatial_support"].fillna(False).astype(bool).values
    enough_neighbors = out["neighbor_count"].fillna(0).astype(float).values >= float(min_neighbors)

    conf = np.zeros(len(out), dtype=float)
    conf[wet] = 0.50
    conf[wet & temporal] += 0.25
    conf[wet & spatial] += 0.25
    # If spatial information is unavailable but the wet decision is temporally
    # persistent, give a small confidence credit rather than rejecting isolated
    # links outright.
    conf[wet & temporal & (~enough_neighbors)] += 0.10
    out["retrieval_confidence"] = np.clip(conf, 0.0, 1.0)

    return out


def plot_cml_retrieval_diagnostic(
    df_rate: pd.DataFrame,
    link_id: Optional[str] = None,
    title_suffix: str = "",
    wet_thr_db: Optional[float] = None,
    rain_col: str = "R_mm_per_h",
    save_path: Optional[str] = None,
):
    """
    Plot signal/baseline, wet flags, attenuation, and rainfall for one link.

    This is intended for internal method-checking slides. It can compare the old
    baseline-first method against the revised statistics-based wet/dry-first
    method for the same selected links.
    """
    import matplotlib.pyplot as plt

    if rain_col not in df_rate.columns:
        raise KeyError(f"{rain_col} not found. Available columns: {df_rate.columns.tolist()}")

    if link_id is None:
        rain_by_link = (
            df_rate.groupby("link_id")[rain_col]
            .sum(min_count=1)
    ...
    d = df_rate[df_rate["link_id"] == link_id].copy().sort_values("time")
    if d.empty:
        raise ValueError(f"No rows found for link_id={link_id}")
    ...
    axes[0].plot(d["time"], d["sig_db"], marker=".", linewidth=1, label="sig_db")
    axes[0].plot(d["time"], d[baseline_col], linewidth=2, label=baseline_col)
    axes[0].set_ylabel("Signal / baseline (dB)")
    axes[0].set_title(f"CML retrieval diagnostic: {title_suffix}\n{link_id}")
    axes[0].legend()
    axes[0].grid(True, alpha=0.3)

    # Show both initial and final wet flags when available.
    if "wet_stat_initial" in d.columns:
        axes[1].step(d["time"], d["wet_stat_initial"].astype(int), where="post", linewidth=1.2, label="wet_stat_initial")
    wet_col = "wet_final" if "wet_final" in d.columns else "wet_rl"
    axes[1].step(d["time"], d[wet_col].astype(int), where="post", linewidth=1.7, label=wet_col)
    if "wet_temporal_support" in d.columns:
        axes[1].step(d["time"], d["wet_temporal_support"].astype(int), where="post", linewidth=1.0, alpha=0.7, label="temporal_support")
    ...
    axes[2].plot(d["time"], d["A_obs_dB"], marker=".", linewidth=1, label="A_obs_dB")
    if wet_thr_db is not None:
        axes[2].axhline(wet_thr_db, linestyle="--", linewidth=1, label=f"wet threshold = {wet_thr_db} dB")
    axes[2].set_ylabel("Attenuation (dB)")
    axes[2].legend()
    axes[2].grid(True, alpha=0.3)

    axes[3].bar(d["time"], d[rain_col], width=0.008, label=rain_col)
    ...
    if save_path:
        ...
        print("Saved diagnostic plot:", save_path)
    plt.show()
    return link_id

# =============================================================================
# 8. Optional grid diagnostic plots
# =============================================================================
...
def plot_cml_grid_diagnostics(
    ...
    *,
    ...
    This is an internal visual check to inspect:
      - primary scientific rainfall field
      - optional display rainfall field
      - CML support confidence
      - CML support mask
      - CML coverage quality

    It helps verify whether the new wet/dry retrieval and gridding/support logic
    produce reasonable map structures before exporting or sharing outputs.
    """
    ...
    meta_plot["lon_mid"] = 0.5 * (meta_plot["XStart"] + meta_plot["XEnd"])
    meta_plot["lat_mid"] = 0.5 * (meta_plot["YStart"] + meta_plot["YEnd"])
    ...
    t_used_main = pd.to_datetime(grid_ds["time"].sel(time=t0, method="nearest").values)
    ...
    if save_path is not None:
        ...
        print("Saved grid diagnostic plot:", save_path)

    plt.show()


# =============================================================================
# Optional: plot daily mean gridded rain rate
# =============================================================================

def plot_daily_mean_cml_rain_rate(
    grid_ds,
    meta_xy_grid,
    day=None,
    *,
    ...
    """
    Plot daily mean CML rain rate.

    Notes
    -----
    R_mm_per_h is a rain-rate field in mm/h.
    Averaging over all 15-min maps gives the mean rain rate for the day,
    still in mm/h.

    This is different from daily accumulation. For daily accumulation,
    each 15-min rain rate should be multiplied by 0.25 h and summed.
    """

    if rain_var not in grid_ds.data_vars:
        raise KeyError(f"{rain_var} not found in grid_ds. Available: {list(grid_ds.data_vars)}")

    ds = grid_ds.copy()

    # Select one day if requested
    if day is not None:
        day_date = pd.Timestamp(day).date()
        times = pd.to_datetime(ds["time"].values)
        mask_day = np.array([t.date() == day_date for t in times])

        if mask_day.sum() == 0:
            raise ValueError(f"No grid times found for day={day_date}")

        ds = ds.sel(time=times[mask_day])

    # Mean over all available 15-min maps
    daily_mean = ds[rain_var].mean(dim="time", skipna=True)

    # Link geometry overlay
    meta_plot = (
        meta_xy_grid[["ID", "XStart", "YStart", "XEnd", "YEnd"]]
        .drop_duplicates("ID")
        .copy()
    ...
    cbar = fig.colorbar(pcm, ax=ax, shrink=0.75, pad=0.02)
    cbar.set_label("Daily mean rain rate (mm h$^{-1}$)")
    ...
    ax.set_title(
        f"Daily mean CML rain rate\n{t0.date()} | {n_times} × 15-min maps",
        fontsize=13,
    )
    ...
    if save_path is not None:
        ...
        save_path.parent.mkdir(parents=True, exist_ok=True)
        plt.savefig(save_path, dpi=180, bbox_inches="tight")
        print("Saved daily mean rain-rate plot:", save_path)
...
# =============================================================================
# Optional: plot CML daily-equivalent rainfall in Ghana map context
# =============================================================================

def plot_daily_equivalent_cml_rainfall(
    grid_ds,
    meta_xy_grid,
    day=None,
    *,
    ...
    """
    Plot CML daily-equivalent rainfall in mm/day on a Ghana Cartopy map.

    Method
    ------
    The CML retrieval gives 15-min rain rates in mm/h.
    To compare with Giovanni/IMERG daily mean precipitation rate in mm/day,
    we compute:

        daily_equivalent_mm_day = mean(R_mm_per_h over available 15-min maps) * 24

    This is a daily-equivalent field, not a strict accumulation, when fewer than
    96 15-min maps are available.
    """
    ...
    if day is not None:
        ...
        day_date = pd.to_datetime(ds["time"].values[0]).date()

    n_times = ds.sizes["time"]

    # Daily-equivalent rainfall in mm/day
    daily_equiv = ds[rain_var].mean(dim="time", skipna=True) * 24.0
    ...
    cbar = fig.colorbar(pcm, ax=ax, shrink=0.75, pad=0.02)
    cbar.set_label("Daily-equivalent rainfall (mm day$^{-1}$)")

    ax.set_title(
        f"CML daily-equivalent rainfall\n"
        f"{day_date} | mean of {n_times} × 15-min rain rates × 24 h",
        fontsize=13,
    )
    ...
    if save_path is not None:
        ...
        save_path.parent.mkdir(parents=True, exist_ok=True)
        plt.savefig(save_path, dpi=180, bbox_inches="tight")
        print("Saved daily-equivalent rainfall plot:", save_path)
