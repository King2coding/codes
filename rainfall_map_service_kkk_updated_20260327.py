    ...
    def run(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for file_path in recent_files:
                frame = pipeline.cml2metadata_coupling_framework(str(file_path), matched_metadata)
            ...
            rain_da, _ = pipeline.grid_rain_15min_rainlink_ok_full_ghana(
            df_s5,
            meta_xy_grid,
            grid_res_deg=self.config["grid_resolution_deg"],
            domain_pad_deg=self.config["domain_pad_deg"],   # kept but ignored when fixed_extent is used
            fixed_extent=(-4.0, 1.25, 4.5, 11.25),            # full Ghana AOI; (lon_min, lon_max, lat_min, lat_max)
            wet_thr=1.0,
            dry_thr=0.0,
            ok_model="exponential",
            ok_range_km=15.0,
            ...
            message = f"Generated {len(generated_files)} rainfall artifact(s) for {latest_dt.isoformat()}."
            return self._result(
                "success",
                message,
