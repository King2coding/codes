...
def plot_lut_histograms_by_hemisphere(hem, seasn, var,
    ...
    for id in surface_type_mapping.keys():
        if id in limb_bin_list_by_srftype.keys():
            ...
            for b in [0, 10, 50, 100, 400]:
                beam_pos = b
                plt_nme = f'LUT_method_line_plot_histogram_{var}_{hem}_{seasn}_{lat_wind}_{sftyp}_{beam_pos}_{cde_run_dte}.png'
                plt_nme = os.path.join(plot_dir, plt_nme)
                ttle = f"{hem.upper()} {seasn.capitalize()} 1998 IR TB Distribution for {var} \n beam position={beam_pos} (+/- 10) \n latitude window = {lat_wind} \n surface type= {sftyp}"
